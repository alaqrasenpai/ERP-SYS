    ...
    <div class="flex-1 flex overflow-hidden relative">
      ...
      <div class="w-[420px] bg-white dark:bg-gray-800 border-e border-gray-100 dark:border-gray-700 shadow-[4px_0_24px_rgba(0,0,0,0.02)] flex flex-col z-10 shrink-0">
        ...
        <div class="p-5 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-900/50">
          ...
          <button @click="showAddCustomerModal = true" class="px-4 py-1.5 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold rounded-lg transition-colors shadow-sm shadow-emerald-500/20">
          ...
          <ul v-else class="space-y-3">
            <li v-for="(item, index) in cart" :key="item.productId" class="bg-white dark:bg-gray-800 p-3 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col">
              <div class="flex justify-between items-start mb-2">
                <div>
                  <h4 class="text-sm font-bold text-gray-900 dark:text-white leading-tight">{{ item.nameAr || item.nameEn }}</h4>
                  <div class="text-xs text-gray-400 mt-0.5">{{ item.price.toFixed(2) }} {{ currency }}</div>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 p-5 border-t border-gray-100 dark:border-gray-700 shadow-[0_-10px_30px_rgba(0,0,0,0.02)] shrink-0">
          ...
          <button @click="checkout" :disabled="cart.length === 0 || !shiftOpen || loading" 
                  class="w-full py-4 bg-[#81C7A1] disabled:bg-gray-200 disabled:text-gray-400 text-white rounded-xl text-base font-black tracking-wide shadow-lg hover:bg-emerald-400 transition-colors flex justify-center items-center">
      ...
      <div class="flex-1 overflow-y-auto p-6 relative">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-5">
          <div v-for="product in filteredProducts" :key="product._id" @click="addToCart(product)" 
            ...
            <!-- 3D Box Icon -->
            <div class="w-12 h-12 mx-auto mb-3 text-emerald-500 group-hover:scale-110 transition-transform">
            ...
            <div class="text-center flex-1 flex flex-col justify-end">
              <h3 class="text-sm font-black text-gray-900 dark:text-white leading-tight mb-1">{{ product.nameAr || product.nameEn }}</h3>
              ...
              <div class="text-base font-black text-emerald-600 mb-2">{{ product.price.toFixed(2) }} {{ currency }}</div>
              
              <div class="mt-auto flex justify-center">
                <span class="text-[10px] font-black px-3 py-1 rounded-full border" 
                      :class="product.stockQuantity > 5 ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : (product.stockQuantity > 0 ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-rose-50 text-rose-600 border-rose-100')">
      ...
      <table v-if="lastOrder" class="w-full mb-2">
        ...
        <tbody>
          <tr v-for="item in lastOrder.items" :key="item.productId">
            <td class="py-1">{{ item.nameAr || item.nameEn }}</td>
...
const fetchProducts = async () => {
  try {
    ...
    products.value = await $api('/restaurant/menu')
    // Transform menu items to match POS product structure
    products.value = products.value.map(p => ({
      ...
      barcode: p.barcode || '',
      price: p.price,
      stockQuantity: 999, // Restaurant items don't have stock quantity in POS view usually
      category: p.category
    }))
  } catch (error) {
...
const checkout = async () => {
  ...
  loading.value = true
  try {
    // For restaurant, we post to /restaurant/orders
    const orderType = route.query.tableId ? 'Dine-In' : 'Takeaway';
    const payload = {
      orderType,
      tableId: route.query.tableId || null,
    ...
    lastOrder.value = await $api('/restaurant/orders', {
      method: 'POST',
      body: payload
    })
