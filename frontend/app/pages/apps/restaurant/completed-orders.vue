<template>
  <div class="p-6 sm:p-8 space-y-8 bg-gray-50 dark:bg-gray-900/50 min-h-screen">
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 gap-4 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
      <div>
        <h2 class="text-2xl sm:text-3xl font-black text-gray-900 dark:text-white tracking-tight flex items-center gap-3">
        ...
        <p class="text-gray-500 dark:text-gray-400 text-sm mt-2 font-medium">{{ $t('restaurant.completed_orders_desc') }}</p>
      </div>
      <div class="flex flex-col sm:flex-row gap-3">
        <div class="flex items-center gap-2 bg-gray-50 dark:bg-gray-900 px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-xl focus-within:ring-2 focus-within:ring-indigo-500 transition-all">
          ...
          <input type="text" v-model="filterCustomer" @keyup.enter="fetchOrders" placeholder="بحث باسم الزبون..." class="border-none focus:ring-0 p-0 font-bold text-sm text-gray-700 dark:text-gray-300 bg-transparent outline-none w-32 sm:w-48">
        </div>
        <div class="flex items-center gap-2 bg-gray-50 dark:bg-gray-900 px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-xl focus-within:ring-2 focus-within:ring-indigo-500 transition-all">
          <span class="text-sm font-bold text-gray-600 dark:text-gray-400">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 flex items-center justify-between">
        <div>
          <p class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">إجمالي المبيعات (حسب الفلتر)</p>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 flex items-center justify-between">
        <div>
          <p class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">عدد الطلبات (حسب الفلتر)</p>
          <p class="text-2xl font-black text-indigo-600">{{ orders.length }}</p>
        </div>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-700">
          <thead class="bg-gray-50 dark:bg-gray-900">
            <tr>
              <th scope="col" class="px-6 py-4 text-start text-xs font-black text-gray-500 dark:text-gray-400 uppercase">{{ $t('restaurant.order_number') }}</th>
              <th scope="col" class="px-6 py-4 text-start text-xs font-black text-gray-500 dark:text-gray-400 uppercase">الزبون</th>
              <th scope="col" class="px-6 py-4 text-start text-xs font-black text-gray-500 dark:text-gray-400 uppercase">{{ $t('restaurant.time') }}</th>
              <th scope="col" class="px-6 py-4 text-start text-xs font-black text-gray-500 dark:text-gray-400 uppercase">{{ $t('restaurant.order_type') }}</th>
              <th scope="col" class="px-6 py-4 text-start text-xs font-black text-gray-500 dark:text-gray-400 uppercase">{{ $t('restaurant.items') }}</th>
              <th scope="col" class="px-6 py-4 text-start text-xs font-black text-gray-500 dark:text-gray-400 uppercase">{{ $t('restaurant.total_amount') }}</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
            <tr v-for="order in paginatedOrders" :key="order._id" class="hover:bg-gray-50 dark:bg-gray-900 transition-colors">
            ...
            <tr v-if="paginatedOrders.length === 0">
              <td colspan="6" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400 text-sm font-medium">
                {{ $t('common.no_data') }}
              </td>
            </tr>
          </tbody>
