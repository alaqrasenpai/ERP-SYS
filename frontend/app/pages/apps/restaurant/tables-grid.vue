  ...
  <div class="p-4 sm:p-8">
    <div class="max-w-7xl mx-auto space-y-6">
      <div class="flex justify-between items-center">
        <div>
          <h2 class="text-2xl font-black text-gray-900 dark:text-white">{{ $t('restaurant.tables_grid') }}</h2>
          <p class="text-sm text-gray-500 dark:text-gray-400">{{ $t('restaurant.description') }}</p>
        </div>
        <div class="flex space-x-3 rtl:space-x-reverse">
          <NuxtLink to="/apps/restaurant/sections" class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 px-6 py-2.5 rounded-xl font-bold shadow-sm hover:bg-gray-50 dark:bg-gray-900 transition flex items-center">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        <button v-for="table in tables" :key="table._id" @click="openTable(table)"
                ...
                :class="table.status === 'Available' ? 'bg-white dark:bg-gray-800 border-green-100 hover:border-green-300' : 'bg-indigo-50 border-indigo-200 hover:border-indigo-400'">
          ...
          <h3 class="text-2xl font-black text-gray-900 dark:text-white">{{ table.tableNumber }}</h3>
          <p class="text-xs font-bold text-gray-500 dark:text-gray-400 mt-1">{{ table.capacity }} {{ $t('restaurant.capacity') }}</p>
          
          <button @click.stop="openEditModal(table)" class="absolute top-3 start-3 w-8 h-8 rounded-full bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm flex items-center justify-center text-gray-400 hover:text-gray-900 dark:text-white hover:border-gray-300 dark:border-gray-600 transition opacity-0 group-hover:opacity-100">
        ...
        <!-- Empty State -->
        <div v-if="!tables.length && !loading" class="col-span-full py-20 text-center bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm">
          <svg class="w-12 h-12 text-gray-300 mb-3 mx-auto block" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>
          <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-1">{{ $t('restaurant.no_tables') }}</h3>
        </div>
      </div>
    </div>
...
const openTable = (table) => {
  router.push({ path: '/apps/restaurant/pos-terminal', query: { tableId: table._id } })
}
