  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 sm:p-8">
    <div class="max-w-6xl mx-auto space-y-6">
      ...
      <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 ">
        <div>
          <h2 class="text-2xl font-black text-gray-900 dark:text-white tracking-tight truncate">{{ $t('hr.pending_attendance_title') }}</h2>
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $t('hr.pending_attendance_desc') }}</p>
        </div>
      </div>
      ...
      <div v-else class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
        <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-700">
          <thead class="bg-gray-50 dark:bg-gray-900">
            <tr>
              <th class="px-6 py-4 text-start text-xs font-black text-gray-500 dark:text-gray-400 uppercase">{{ $t('hr.device') }}</th>
              <th class="px-6 py-4 text-start text-xs font-black text-gray-500 dark:text-gray-400 uppercase">{{ $t('hr.fingerprint_id') }}</th>
              <th class="px-6 py-4 text-start text-xs font-black text-gray-500 dark:text-gray-400 uppercase">{{ $t('hr.record_time') }}</th>
              <th class="px-6 py-4 text-end text-xs font-black text-gray-500 dark:text-gray-400 uppercase">{{ $t('debts.actions') }}</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
            <tr v-for="log in logs" :key="log._id">
              <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900 dark:text-white">{{ log.deviceId?.deviceName || 'Unknown' }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white font-bold">#{{ log.deviceEmployeeNumber }}</td>
            ...
            <tr v-if="logs.length === 0">
              <td colspan="4" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400 font-bold">{{ $t('hr.no_pending') }}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div v-if="showModal" class="fixed inset-0 bg-gray-900/50 flex items-center justify-center p-4 z-50">
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md overflow-hidden text-start rtl:text-right" :dir="$i18n.locale === 'ar' ? 'rtl' : 'ltr'">
          <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-900">
            <h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ $t('hr.link_title') }}</h3>
          ...
          <form @submit.prevent="linkEmployee" class="p-6 space-y-4">
            <div>
              ...
              <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{{ $t('hr.select_employee_label') }}</label>
              <select v-model="selectedEmployeeId" required class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-indigo-500">
            ...
            <div class="pt-4 flex justify-end gap-3 rtl:space-x-reverse" :dir="$i18n.locale === 'ar' ? 'ltr' : 'rtl'">
              ...
              <button type="button" @click="showModal = false" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl font-medium text-gray-700 dark:text-gray-300">{{ $t('common.cancel') }}</button>
            </div>
          </form>
        </div>
      </div>
