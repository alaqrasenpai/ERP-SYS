<template>
  <div class="min-h-screen bg-gray-100 dark:bg-gray-700 p-4 sm:p-8">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="customer in filteredCustomers" :key="customer._id" class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow p-6 relative">
        <div class="flex justify-between items-start mb-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-sm text-gray-500 dark:text-gray-400">{{ customer.phone || $t('crm.no_phone') }}</p>
            </div>
          </div>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-900 flex-shrink-0">
          <h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ isEditing ? $t('crm.edit_customer') : $t('crm.add_customer') }}</h3>
        ...
        <div class="p-6 overflow-y-auto flex-1">
          <form @submit.prevent="saveCustomer" class="space-y-4">
            <div>
              <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{{ $t('crm.full_name') }}</label>
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{{ $t('crm.phone') }}</label>
              ...
              <div>
                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{{ $t('crm.email') }}</label>
            ...
            <div>
              <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{{ $t('crm.address') }}</label>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t">
              <button type="button" @click="showModal = false" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl text-gray-700 dark:text-gray-300 font-medium hover:bg-gray-50 dark:bg-gray-900">{{ $t('crm.cancel') }}</button>
    ...
    <div v-if="showDebtModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div class="flex-1 overflow-y-auto p-6 bg-gray-50 dark:bg-gray-900">
          <h4 class="text-sm font-bold text-gray-700 dark:text-gray-300 uppercase mb-3">{{ $t('crm.ledger_history') }}</h4>
          <div v-if="debtLogs.length === 0" class="text-center p-4 text-gray-500 dark:text-gray-400 text-sm">{{ $t('crm.no_debt_records') }}</div>
          <div class="space-y-3">
            <div v-for="log in debtLogs" :key="log._id" class="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm flex justify-between items-center">
              <div>
                <p class="font-bold text-gray-900 dark:text-white text-sm flex items-center">
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ new Date(log.date).toLocaleString() }} &bull; {{ log.notes || (log.orderId ? `Order ${log.orderId.orderNumber}` : $t('crm.manual_entry')) }}</p>
                <p class="text-xs font-medium text-gray-500 dark:text-gray-400 mt-0.5">{{ $t('crm.method') }} {{ log.paymentMethod }}</p>
              </div>
              <div class="text-end">
        ...
        <div class="p-6 bg-white dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700">
          <form @submit.prevent="submitDebtAdjustment" class="space-y-4">
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{{ $t('crm.transaction_type') }}</label>
                <select v-model="debtForm.type" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-indigo-500 focus:border-indigo-500">
                  <option value="payment">{{ $t('crm.receive_payment') }}</option>
                  <option value="charge">{{ $t('crm.manual_charge') }}</option>
                </select>
              </div>
              <div>
                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{{ $t('crm.amount') }}</label>
                <input v-model="debtForm.amount" type="number" step="0.01" min="0.01" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-indigo-500 focus:border-indigo-500">
              </div>
            </div>
            <div class="grid grid-cols-2 gap-4">
               <div>
                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{{ $t('crm.payment_method') }}</label>
                <select v-model="debtForm.paymentMethod" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-indigo-500 focus:border-indigo-500">
                  <option value="Cash">{{ $t('crm.cash') }}</option>
                  <option value="Card">{{ $t('crm.bank_transfer') }}</option>
                </select>
              </div>
              <div>
                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{{ $t('crm.notes') }}</label>
                <input v-model="debtForm.notes" type="text" :placeholder="$t('crm.notes_placeholder')" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-indigo-500 focus:border-indigo-500">
              </div>
            </div>
