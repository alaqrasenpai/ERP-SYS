  ...
  <div class="p-4 sm:p-8">
    <div class="max-w-7xl mx-auto space-y-6">
      <div class="flex justify-between items-center">
        <div>
          <h2 class="text-2xl font-black text-gray-900 dark:text-white">{{ $t('restaurant.menu_setup') }}</h2>
          <p class="text-sm text-gray-500 dark:text-gray-400">{{ $t('restaurant.description') }}</p>
        </div>
        <div class="flex items-center gap-3">
          <NuxtLink to="/dashboard/restaurant/categories" class="px-4 py-2 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm hover:bg-gray-50 dark:bg-gray-900 transition font-bold flex items-center gap-2">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
        <div v-for="item in menu" :key="item._id" class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden hover:shadow-md transition">
          <div class="h-40 bg-gray-50 dark:bg-gray-900 relative group">
            ...
            <div class="absolute top-2 end-2 flex gap-1">
              <button @click.stop="editDish(item)" class="bg-white dark:bg-gray-800 text-indigo-600 hover:text-indigo-800 p-1.5 rounded-full shadow-sm flex items-center justify-center">
          ...
          <div class="p-4">
            <h3 class="font-bold text-gray-900 dark:text-white truncate">{{ locale === 'ar' ? item.nameAr : item.nameEn }}</h3>
            <p class="text-xs text-gray-500 dark:text-gray-400 mb-2 truncate">{{ locale === 'ar' ? item.categoryAr : item.categoryEn }}</p>
        ...
        <!-- Empty State -->
        <div v-if="!menu.length && !loading" class="col-span-full p-12 text-center text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 border-dashed">
