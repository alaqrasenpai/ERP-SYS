        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                <th scope="col" class="px-6 py-4 text-start text-xs font-black text-gray-500 uppercase tracking-wider">{{ $t('restaurant.provider_name_en') }}</th>
                <th scope="col" class="px-6 py-4 text-start text-xs font-black text-gray-500 uppercase tracking-wider">{{ $t('restaurant.provider_name_ar') }}</th>
                <th scope="col" class="px-6 py-4 text-start text-xs font-black text-gray-500 uppercase tracking-wider">{{ $t('restaurant.commission_rate') }}</th>
                <th scope="col" class="px-6 py-4 text-start text-xs font-black text-gray-500 uppercase tracking-wider">{{ $t('restaurant.account_balance') }}</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="provider in providers" :key="provider._id" class="hover:bg-gray-50 transition">
                <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">{{ provider.nameEn }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-600" dir="rtl">{{ provider.nameAr }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-mono">{{ provider.commissionRate }}%</td>
          ...
          <div v-if="!providers.length && !loading" class="p-8 text-center text-gray-500 text-sm">
    ...
    <div v-if="showModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center p-4 z-50 print:hidden">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
          <h3 class="text-lg font-bold text-gray-900">{{ $t('restaurant.add_provider') }}</h3>
