<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 sm:bg-gray-100 dark:bg-gray-700 p-4 sm:p-8">
    <div class="max-w-6xl mx-auto space-y-6">
      <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 ">
        <div>
        <h2 class="text-2xl font-black text-gray-900 dark:text-white tracking-tight truncate">{{ $t('users.title') }}</h2>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $t('users.description') }}</p>
      </div>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-700">
          <thead class="bg-gray-50 dark:bg-gray-900">
            <tr>
              <th scope="col" class="px-4 py-3 sm:px-6 sm:py-4 text-start text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ $t('users.staff_details') }}</th>
              <th scope="col" class="px-4 py-3 sm:px-6 sm:py-4 text-start text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ $t('users.assigned_role') }}</th>
              <th scope="col" class="px-4 py-3 sm:px-6 sm:py-4 text-start text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ $t('users.linked_employee') }}</th>
              <th scope="col" class="px-4 py-3 sm:px-6 sm:py-4 text-start text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ $t('users.status') }}</th>
              <th scope="col" class="px-4 py-3 sm:px-6 sm:py-4 text-end text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ $t('users.actions') }}</th>
            </tr>
        </thead>
        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
          <tr v-for="user in users" :key="user._id" class="hover:bg-gray-50 dark:bg-gray-900 transition-colors">
            ...
            <td class="px-4 py-3 sm:px-6 sm:py-4 whitespace-nowrap">
              <span v-if="user.employeeId" class="text-sm font-bold text-gray-900 dark:text-white flex items-center">
          ...
          <tr v-if="users.length === 0">
            <td colspan="4" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400 text-sm font-bold">{{ $t('users.no_staff_found') }}</td>
          </tr>
        </tbody>
        </table>
      </div>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-gray-900/50 flex items-center justify-center p-4 z-50">
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-900">
          <h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ editingUser ? $t('users.edit_staff') : $t('users.add_staff') }}</h3>
        ...
        <form @submit.prevent="saveUser" class="flex flex-col">
          <div class="p-6 space-y-4">
            <div>
              <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{{ $t('users.full_name') }}</label>
              <input v-model="form.name" type="text" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" required>
            </div>
            <div>
              <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{{ $t('users.email_address') }}</label>
              <input v-model="form.email" type="email" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" required>
            </div>
            <div>
              <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">
                {{ $t('users.password') }} <span v-if="editingUser" class="text-xs text-gray-400 font-normal">{{ $t('users.leave_blank') }}</span>
              </label>
              <input v-model="form.password" type="password" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" :required="!editingUser">
            </div>
            <div>
              <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{{ $t('users.assigned_role') }}</label>
              <select v-model="form.roleId" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" required>
                <option value="" disabled>{{ $t('users.select_role') }}</option>
                <option v-for="role in roles" :key="role._id" :value="role._id">{{ role.name }}</option>
              </select>
            </div>
            <div>
              <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{{ $t('users.linked_employee') }}</label>
              <select v-model="form.employeeId" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
          ...
          <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 flex justify-end gap-3">
            <button type="button" @click="showModal = false" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl text-gray-700 dark:text-gray-300 font-medium hover:bg-gray-50 dark:bg-gray-900 transition-colors">{{ $t('users.cancel') }}</button>
