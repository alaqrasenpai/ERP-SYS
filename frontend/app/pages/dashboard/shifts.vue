  ...
  <div class="min-h-screen bg-[#FAFAFA] p-4 sm:p-8 font-sans dir-rtl">
    <div class="max-w-7xl mx-auto space-y-8">
      ...
      <div class="text-center pt-4 pb-2">
        <h2 class="text-3xl font-black text-gray-900 dark:text-white tracking-tight">{{ $t('cash_shifts_page.title', 'إدارة الورديات والقاصة') }}</h2>
      ...
      <div class="flex flex-col lg:flex-row gap-8 items-start justify-center">
        ...
        <div class="w-full lg:w-[400px] bg-white dark:bg-gray-800 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 dark:border-gray-700 overflow-hidden shrink-0">
          <div class="p-6 text-center border-b border-gray-50 bg-gray-50 dark:bg-gray-900/50">
            <h3 class="text-base font-black text-gray-900 dark:text-white">{{ $t('cash_shifts_page.shift_details', 'تفاصيل الوردية') }}</h3>
          </div>
          ...
          <div class="p-6 space-y-6">
            ...
            <template v-else-if="currentOpenShift">
              ...
              <div class="space-y-4">
                <div class="flex justify-between items-center text-sm">
                  <span class="text-gray-500 dark:text-gray-400 font-bold">{{ $t('cash_shifts_page.identifier', 'المعرف') }}</span>
                  <span class="font-bold text-gray-900 dark:text-white">{{ currentOpenShift._id.substring(0, 12) }}</span>
                </div>
                <div class="flex justify-between items-center text-sm">
                  <span class="text-gray-500 dark:text-gray-400 font-bold">{{ $t('cash_shifts_page.start_time', 'وقت البدء') }}</span>
                  <span class="font-bold text-gray-900 dark:text-white">{{ new Date(currentOpenShift.openedAt).toLocaleString() }}</span>
                </div>
                <div class="flex justify-between items-center text-sm pt-2">
                  <span class="text-gray-500 dark:text-gray-400 font-bold">{{ $t('cash_shifts_page.starting_cash', 'كاش البداية') }}</span>
                ...
                <div class="flex justify-between items-center text-sm pt-2 pb-4 border-b border-gray-100 dark:border-gray-700">
                  <span class="text-gray-500 dark:text-gray-400 font-bold">{{ $t('cash_shifts_page.expected_in_safe', 'المتوقع في القاصة') }}</span>
              ...
              <div class="pt-6 mt-6 border-t border-gray-100 dark:border-gray-700">
                ...
                <div class="space-y-2">
                  <label class="block text-xs font-bold text-gray-500 dark:text-gray-400">{{ $t('cash_shifts_page.actual_cash', 'الموجود فعلياً') }}</label>
                  <div class="relative">
                    <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                      <span class="text-gray-400 font-bold">{{ useCookie('erp_currency').value || 'SAR' }}</span>
                    </div>
                    <input type="number" v-model.number="actualCash" :placeholder="$t('cash_shifts_page.count_cash_placeholder', 'قم بعد النقدية وأدخل المبلغ هنا...')" class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white text-sm font-bold rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block pr-12 p-3.5 transition-colors">
                  </div>
                </div>
            ...
            <template v-else>
              <div class="text-center py-8">
                <div class="w-16 h-16 bg-gray-100 dark:bg-gray-700 rounded-full flex items-center justify-center mx-auto mb-4 border border-gray-200 dark:border-gray-700 border-dashed">
                ...
                <h4 class="text-gray-900 dark:text-white font-black mb-2">{{ $t('cash_shifts_page.no_active_shift', 'لا توجد وردية نشطة') }}</h4>
                <p class="text-xs text-gray-500 dark:text-gray-400 font-bold mb-6">{{ $t('cash_shifts_page.open_shift_to_start', 'الرجاء فتح وردية جديدة للبدء باستقبال الطلبات والمبيعات.') }}</p>
                
                <div class="space-y-4">
                  <div>
                    <label class="block text-xs font-bold text-gray-500 dark:text-gray-400 text-start mb-2">{{ $t('cash_shifts_page.starting_cash_drawer', 'كاش البداية (الموجود في الدرج)') }}</label>
                    <div class="relative">
                      <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                        <span class="text-gray-400 font-bold">{{ useCookie('erp_currency').value || 'SAR' }}</span>
                      </div>
                      <input type="number" v-model.number="openingCash" :placeholder="$t('cash_shifts_page.example_100', 'مثال: 100')" class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white text-sm font-bold rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block pr-12 p-3.5 transition-colors">
                    </div>
                  </div>
        ...
        <div class="flex-1 w-full bg-white dark:bg-gray-800 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 dark:border-gray-700 overflow-hidden">
          <div class="p-6 text-center border-b border-gray-50 bg-gray-50 dark:bg-gray-900/50">
            <h3 class="text-base font-black text-gray-900 dark:text-white">{{ $t('cash_shifts_page.shifts_history', 'سجل الورديات') }}</h3>
          </div>
          
          <div class="overflow-x-auto">
            <table class="w-full text-sm text-right text-gray-500 dark:text-gray-400">
              <thead class="text-xs text-gray-400 font-bold uppercase bg-gray-50 dark:bg-gray-900/50 border-b border-gray-100 dark:border-gray-700">
                <tr>
              ...
              <tbody>
                <tr v-for="shift in closedShifts" :key="shift._id" class="bg-white dark:bg-gray-800 border-b border-gray-50 hover:bg-gray-50 dark:bg-gray-900/50 transition-colors">
                  <td class="px-6 py-4">
                    <div class="font-black text-gray-900 dark:text-white">{{ shift._id.substring(0, 10) }}</div>
