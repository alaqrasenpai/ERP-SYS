<template>
  <div class="min-h-screen bg-gray-100 p-4 sm:p-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
      <h2 class="text-2xl font-black text-gray-900 tracking-tight">Check Payments Portal</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div class="bg-orange-50 rounded-xl p-4 border border-orange-100 shadow-sm">
        <p class="text-xs font-bold text-orange-600 uppercase">Pending Checks</p>
      ...
      <div class="bg-emerald-50 rounded-xl p-4 border border-emerald-100 shadow-sm">
        <p class="text-xs font-bold text-emerald-600 uppercase">Collected Total</p>
      ...
      <div class="bg-red-50 rounded-xl p-4 border border-red-100 shadow-sm">
        <p class="text-xs font-bold text-red-600 uppercase">Bounced Count</p>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="check in checks" :key="check._id" class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 relative">
        <div class="absolute top-4 right-4">
          <span v-if="check.status === 'Pending'" class="px-2.5 py-1 bg-orange-100 text-orange-700 text-xs font-bold rounded-full">Pending</span>
          <span v-else-if="check.status === 'Collected'" class="px-2.5 py-1 bg-emerald-100 text-emerald-700 text-xs font-bold rounded-full">Collected</span>
          <span v-else class="px-2.5 py-1 bg-red-100 text-red-700 text-xs font-bold rounded-full">Bounced</span>
        </div>

        <h3 class="text-xl font-black text-gray-900">${{ check.amount.toFixed(2) }}</h3>
        <p class="text-sm font-bold text-gray-700 mt-2">{{ check.customerId?.name || 'Unknown Customer' }}</p>
        
        <div class="mt-4 space-y-1.5 border-t border-gray-100 pt-3">
          <p class="text-xs text-gray-600"><span class="font-bold text-gray-500 w-20 inline-block">Check No:</span> {{ check.checkNumber }}</p>
          <p class="text-xs text-gray-600"><span class="font-bold text-gray-500 w-20 inline-block">Bank:</span> {{ check.bankName }}</p>
          <p class="text-xs text-gray-600"><span class="font-bold text-gray-500 w-20 inline-block">Due Date:</span> <span :class="isOverdue(check.dueDate) && check.status === 'Pending' ? 'text-red-600 font-bold' : ''">{{ new Date(check.dueDate).toLocaleDateString() }}</span></p>
          <p class="text-xs text-gray-600"><span class="font-bold text-gray-500 w-20 inline-block">Reference:</span> {{ check.orderId?.orderNumber || 'Manual' }}</p>
        </div>
...
const updateStatus = async (checkId, status) => {
  if (!confirm(`Are you sure you want to mark this check as ${status}? This will trigger accounting journal entries.`)) return
