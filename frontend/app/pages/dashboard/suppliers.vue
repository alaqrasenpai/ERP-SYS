<template>
  <div class="min-h-screen bg-gray-100 p-4 sm:p-8">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
      <h2 class="text-2xl font-black text-gray-900 tracking-tight truncate">{{ $t('suppliers.title') }}</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div class="bg-white rounded-xl p-4 border border-gray-100 shadow-sm flex items-center">
        <div class="w-12 h-12 bg-primary-100 text-primary-600 rounded-full flex items-center justify-center me-4">
      ...
      <div v-for="supplier in filteredSuppliers" :key="supplier._id" class="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow p-6 relative">
        <div class="flex justify-between items-start mb-4">
          <div class="flex items-center gap-3">
            <div class="w-12 h-12 bg-primary-100 text-primary-700 rounded-xl flex items-center justify-center font-bold text-xl uppercase">
          ...
          <button @click="openEditModal(supplier)" class="text-gray-400 hover:text-primary-600 transition-colors">
        ...
        <div class="bg-gray-50 rounded-xl p-4 border border-gray-100 flex justify-between items-center">
          <div>
            ...
            <p class="text-xl font-black" :class="supplier.balanceDue > 0 ? 'text-red-600' : 'text-secondary-600'">${{ supplier.balanceDue.toFixed(2) }}</p>
          </div>
        ...
        <div class="p-6">
          <form @submit.prevent="saveSupplier" class="space-y-4">
            ...
            <div>
              ...
              <input v-model="form.contactPerson" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500">
            </div>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t">
              ...
              <button type="submit" class="px-4 py-2 bg-primary-600 text-white rounded-xl font-bold hover:bg-primary-700">{{ $t('suppliers.save_supplier') }}</button>
            </div>
          </form>
        </div>
      </div>
        ...
        <div class="p-6">
          ...
          <form @submit.prevent="submitPayment" class="space-y-4">
            <div>
              ...
              <input v-model="payForm.amount" type="number" step="0.01" min="0.01" :max="activeSupplier?.balanceDue" required class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500 text-lg font-bold text-center">
            </div>
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-1">{{ $t('suppliers.notes_reference') }}</label>
              <input v-model="payForm.notes" type="text" :placeholder="$t('suppliers.e_g_wire_transfer')" class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500">
            </div>
            <div class="pt-2">
              <button type="submit" class="w-full px-4 py-3 bg-secondary-600 text-white rounded-xl font-bold hover:bg-secondary-700 flex justify-center items-center">
