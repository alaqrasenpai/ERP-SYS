    ...
    <div class="max-w-7xl mx-auto space-y-6">
      <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
        ...
        <div class="flex gap-3 mt-4 sm:mt-0 print:hidden">
          ...
          <button @click="exportCSV" class="px-4 py-2 bg-secondary-600 text-white rounded-xl text-sm font-bold hover:bg-secondary-700 shadow-sm transition-colors flex items-center">
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6 print:hidden">
        <nav class="flex divide-x divide-x-reverse divide-gray-100" aria-label="Tabs">
          <button @click="activeReport = 'pnl'" :class="activeReport === 'pnl' ? 'bg-primary-50 text-primary-700' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'" class="flex-1 group relative min-w-0 overflow-hidden py-4 px-4 text-center text-sm font-bold hover:bg-gray-50 focus:z-10 transition-colors">
            <span>{{ $t('reports.profit_and_loss') }}</span>
            <span aria-hidden="true" :class="activeReport === 'pnl' ? 'bg-primary-500' : 'bg-transparent'" class="absolute inset-x-0 bottom-0 h-1"></span>
          </button>
          <button @click="activeReport = 'debtors'" :class="activeReport === 'debtors' ? 'bg-primary-50 text-primary-700' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'" class="flex-1 group relative min-w-0 overflow-hidden py-4 px-4 text-center text-sm font-bold hover:bg-gray-50 focus:z-10 transition-colors">
            <span>{{ $t('reports.debtors_collections') }}</span>
            <span aria-hidden="true" :class="activeReport === 'debtors' ? 'bg-primary-500' : 'bg-transparent'" class="absolute inset-x-0 bottom-0 h-1"></span>
          </button>
          <button @click="activeReport = 'inventory'" :class="activeReport === 'inventory' ? 'bg-primary-50 text-primary-700' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'" class="flex-1 group relative min-w-0 overflow-hidden py-4 px-4 text-center text-sm font-bold hover:bg-gray-50 focus:z-10 transition-colors">
            <span>{{ $t('reports.inventory_valuation') }}</span>
            <span aria-hidden="true" :class="activeReport === 'inventory' ? 'bg-primary-500' : 'bg-transparent'" class="absolute inset-x-0 bottom-0 h-1"></span>
          </button>
        </nav>
      </div>
        ...
        <div v-if="activeReport === 'pnl'" class="space-y-6">
          <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 print:hidden">
            ...
            <div class="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:gap-4 sm: items-end">
              <div class="flex-1">
                ...
                <input v-model="dateRange.start" type="date" class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500">
              </div>
              <div class="flex-1">
                <label class="block text-xs font-bold text-gray-500 uppercase mb-1">{{ $t('reports.end_date') }}</label>
                <input v-model="dateRange.end" type="date" class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500">
              </div>
              <button @click="fetchPnL" class="bg-primary-600 hover:bg-primary-700 transition-colors shadow-sm focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 text-white px-4 py-2 shrink-0 whitespace-nowrap rounded-xl font-bold transition-all shadow-sm">
                {{ $t('reports.generate') }}
              </button>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              ...
              <p class="text-3xl font-black text-secondary-600 mt-2">{{ currency }}{{ (pnl.totalRevenue || 0).toLocaleString('en-US', {minimumFractionDigits: 2}) }}</p>
            ...
            <div class="bg-primary-600 p-6 rounded-2xl shadow-md border border-primary-700 text-white">
              <p class="text-sm font-bold text-primary-200 uppercase">{{ $t('reports.net_profit') }}</p>
              <p class="text-4xl font-black mt-2">{{ currency }}{{ (pnl.netProfit || 0).toLocaleString('en-US', {minimumFractionDigits: 2}) }}</p>
              <p class="text-xs text-primary-200 mt-1">{{ $t('reports.revenue_minus_expenses') }}</p>
            </div>
          </div>
        </div>
              ...
              <tbody class="bg-white divide-y divide-gray-100">
                <tr v-for="customer in debtors" :key="customer._id" class="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td class="px-4 py-3 sm:px-6 sm:py-4 whitespace-nowrap">
                    ...
                    <span v-else class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-secondary-100 text-secondary-800 border border-secondary-200">
                      {{ $t('reports.no_overdue') }}
                    </span>
                  </td>
                  <td class="px-4 py-3 sm:px-6 sm:py-4 whitespace-nowrap text-end text-sm font-medium">
                    <NuxtLink to="/dashboard/customers" class="text-primary-600 hover:text-primary-900 font-bold">{{ $t('reports.go_to_crm') }}</NuxtLink>
                  </td>
                </tr>
