<template>
  <div class="min-h-screen bg-gray-100 p-4 sm:p-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
      <h2 class="text-2xl font-black text-gray-900 tracking-tight">Installment Plans Tracker</h2>
    ...
    <div v-else class="space-y-6">
      <div v-for="plan in plans" :key="plan._id" class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 flex justify-between items-center">
          <div>
            <h3 class="font-bold text-gray-900">{{ plan.customerId?.name || 'Unknown Customer' }}</h3>
            <p class="text-xs text-gray-500 mt-0.5">Order: {{ plan.orderId?.orderNumber }} &bull; Created: {{ new Date(plan.createdAt).toLocaleDateString() }}</p>
          </div>
          <div class="text-right">
            <p class="text-sm font-bold text-gray-600">Total: ${{ plan.totalAmount.toFixed(2) }}</p>
            <p class="text-lg font-black text-indigo-600">Remaining: ${{ plan.remainingAmount.toFixed(2) }}</p>
          </div>
        </div>
        ...
        <div class="p-6">
          <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            <div v-for="(inst, idx) in plan.installments" :key="inst._id" class="border rounded-xl p-3 relative" :class="getInstClass(inst)">
              <div class="absolute -top-2 -left-2 w-5 h-5 bg-gray-900 text-white text-xs font-bold rounded-full flex items-center justify-center">{{ idx + 1 }}</div>
              
              <p class="text-xs font-bold text-gray-500 uppercase mt-1 mb-1">{{ inst.status }}</p>
              <h4 class="text-lg font-black text-gray-900">${{ inst.amount.toFixed(2) }}</h4>
              <p class="text-xs text-gray-600 mt-1" :class="isOverdue(inst.dueDate) && inst.status === 'Pending' ? 'text-red-600 font-bold' : ''">Due: {{ new Date(inst.dueDate).toLocaleDateString() }}</p>
              ...
              <p v-else class="text-xs text-emerald-600 font-bold mt-3 text-center">Paid on {{ new Date(inst.paidDate).toLocaleDateString() }}</p>
            </div>
          </div>
        </div>
      </div>
...
const payInstallment = async (planId, instId) => {
  if (!confirm('Mark this installment as paid? This will debit cash and credit Accounts Receivable.')) return
