  ...
  <div class="min-h-screen bg-gray-100 p-4 sm:p-8">
    <div class="max-w-4xl mx-auto space-y-6">
      <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
        <div>
          <h2 class="text-2xl font-black text-gray-900 tracking-tight truncate">{{ $t('settings.title') }}</h2>
        ...
        <div class="flex items-center gap-3 mt-4 sm:mt-0 w-full sm:w-auto overflow-x-auto pb-2 sm:pb-0">
          ...
          <NuxtLink to="/dashboard" class="px-5 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-xl text-sm font-bold hover:bg-gray-50 shadow-sm transition-colors flex items-center shrink-0 whitespace-nowrap">
          ...
          <button @click="saveSettings" class="px-5 py-2.5 bg-primary-600 hover:bg-primary-700 transition-colors shadow-sm focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 text-white shrink-0 whitespace-nowrap rounded-xl font-bold flex items-center text-sm" :disabled="saving">
      ...
      <div v-else class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="border-b border-gray-100">
          <nav class="-mb-px flex gap-8 px-6" aria-label="Tabs">
            <button @click="activeTab = 'general'" :class="activeTab === 'general' ? 'border-primary-500 text-primary-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'" class="whitespace-nowrap py-4 px-1 border-b-2 font-bold text-sm">
              {{ $t('settings.general_branding') }}
            </button>
            <button @click="activeTab = 'financial'" :class="activeTab === 'financial' ? 'border-primary-500 text-primary-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'" class="whitespace-nowrap py-4 px-1 border-b-2 font-bold text-sm">
              {{ $t('settings.financial_settings') }}
            </button>
            <button @click="activeTab = 'localization'" :class="activeTab === 'localization' ? 'border-primary-500 text-primary-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'" class="whitespace-nowrap py-4 px-1 border-b-2 font-bold text-sm">
              {{ $t('settings.localization') }}
            </button>
            <button @click="activeTab = 'pos'" :class="activeTab === 'pos' ? 'border-primary-500 text-primary-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'" class="whitespace-nowrap py-4 px-1 border-b-2 font-bold text-sm">
              {{ $t('settings.pos_configuration') }}
            </button>
            <button @click="activeTab = 'qrmenu'" :class="activeTab === 'qrmenu' ? 'border-primary-500 text-primary-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'" class="whitespace-nowrap py-4 px-1 border-b-2 font-bold text-sm">
        ...
        <div class="p-6">
          ...
          <div v-if="activeTab === 'general'" class="space-y-6">
            <div>
              ...
              <input v-model="settings.storeName" type="text" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500">
            </div>
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-2">{{ $t('settings.logo_url') }}</label>
              <input v-model="settings.logoUrl" type="text" placeholder="https://..." class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500">
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">{{ $t('settings.primary_phone') }}</label>
                <input v-model="settings.phone" type="text" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500">
              </div>
              <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">{{ $t('settings.business_address') }}</label>
                <textarea v-model="settings.address" rows="2" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500"></textarea>
              </div>
              <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">{{ $t('settings.primary_color') }}</label>
                <div class="flex items-center gap-3">
                  <input v-model="settings.primaryColor" type="color" class="h-10 w-14 rounded cursor-pointer border-0 p-0">
                  <input v-model="settings.primaryColor" type="text" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500 uppercase font-mono text-sm">
                </div>
              </div>
              <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">{{ $t('settings.secondary_color') }}</label>
                <div class="flex items-center gap-3">
                  <input v-model="settings.secondaryColor" type="color" class="h-10 w-14 rounded cursor-pointer border-0 p-0">
                  <input v-model="settings.secondaryColor" type="text" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500 uppercase font-mono text-sm">
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-if="activeTab === 'financial'" class="space-y-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                ...
                <select v-model="settings.currency" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500">
              ...
              <div>
                ...
                <input v-model.number="settings.taxRate" type="number" step="0.1" min="0" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500">
              </div>
              <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">{{ $t('settings.fiscal_year_start') }}</label>
                <input v-model="settings.fiscalYearStart" type="text" placeholder="01-01" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500">
              </div>
            </div>
          ...
          <div v-if="activeTab === 'localization'" class="space-y-6">
            <div>
              ...
              <select v-model="settings.defaultLanguage" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500">
            ...
            <div>
              ...
              <div class="space-y-2">
                <label class="flex items-center gap-3 cursor-pointer rtl:">
                  <input type="checkbox" value="ar" v-model="settings.allowedLanguages" class="w-5 h-5 text-primary-600 rounded border-gray-300">
                  <span class="text-sm font-medium text-gray-900">العربية (Arabic)</span>
                </label>
                <label class="flex items-center gap-3 cursor-pointer rtl:">
                  <input type="checkbox" value="en" v-model="settings.allowedLanguages" class="w-5 h-5 text-primary-600 rounded border-gray-300">
          ...
          <div v-if="activeTab === 'pos'" class="space-y-6">
            <div>
              ...
              <textarea v-model="settings.receiptHeader" rows="2" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500" :placeholder="$t('settings.receipt_header_placeholder')"></textarea>
            </div>
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-2">{{ $t('settings.receipt_footer') }}</label>
              <textarea v-model="settings.receiptFooter" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500" :placeholder="$t('settings.receipt_footer_placeholder')"></textarea>
            </div>
