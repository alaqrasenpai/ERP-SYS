  ...
  <div class="min-h-screen bg-gray-100 p-8">
    <div class="max-w-7xl mx-auto">
      <div class="flex justify-between items-center mb-8">
        <div>
          <h1 class="text-3xl font-bold text-gray-900 tracking-tight">Payroll Processing</h1>
          <p class="text-sm text-gray-500 mt-1">Generate and manage monthly salary sheets.</p>
        </div>
      ...
      <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm mb-8 flex flex-col md:flex-row items-end justify-between gap-4">
        <div>
          <label class="block text-sm font-bold text-gray-700 mb-1">Target Month</label>
      ...
      <div id="payroll-printable-area" class="bg-white shadow-sm border border-gray-200 rounded-xl overflow-hidden">
        <div class="p-4 border-b border-gray-200 bg-gray-50 hidden print:block">
           <h2 class="text-2xl font-bold text-gray-900 text-center">Payroll Sheet - {{ targetMonth }}</h2>
        </div>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-6 py-4 text-right text-xs font-bold text-gray-500 uppercase tracking-wider">Basic ($)</th>
                <th class="px-6 py-4 text-right text-xs font-bold text-gray-500 uppercase tracking-wider">Allowances ($)</th>
                <th class="px-6 py-4 text-right text-xs font-bold text-gray-500 uppercase tracking-wider">Overtime ($)</th>
                <th class="px-6 py-4 text-right text-xs font-bold text-gray-500 uppercase tracking-wider text-red-600">Deductions ($)</th>
                <th class="px-6 py-4 text-right text-xs font-bold text-gray-900 uppercase tracking-wider">Net Salary ($)</th>
                <th class="px-6 py-4 text-center text-xs font-bold text-gray-500 uppercase tracking-wider print:hidden">Status</th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-100">
              <tr v-for="run in filteredPayroll" :key="run._id" class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm font-bold text-gray-900">{{ run.employeeId?.name || 'Unknown' }}</div>
                  <div class="text-xs text-gray-500">{{ run.employeeId?.position }}</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-right font-medium text-gray-600">{{ run.basicSalary.toFixed(2) }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-right font-medium text-green-600">+ {{ run.totalAllowances.toFixed(2) }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-right font-medium text-green-600">
                  <span title="Overtime Hours: " class="cursor-help">+ {{ run.totalOvertimePay.toFixed(2) }}</span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-right font-medium text-red-600">- {{ run.deductions.toFixed(2) }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-right font-black text-gray-900 bg-gray-50">${{ run.netSalary.toFixed(2) }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-center text-sm print:hidden">
                  <button v-if="!run.isPaid" @click="markAsPaid(run._id)" class="px-3 py-1 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 font-bold rounded-md text-xs transition-colors">Mark Paid</button>
              ...
              <tr v-if="filteredPayroll.length === 0">
                <td colspan="8" class="px-6 py-12 text-center text-sm text-gray-500">No payroll data generated for {{ targetMonth }}. Click 'Generate Payroll' to calculate.</td>
              </tr>
            </tbody>
            <tfoot class="bg-gray-50 font-bold" v-if="filteredPayroll.length > 0">
              <tr>
                <td colspan="6" class="px-6 py-4 text-right text-sm text-gray-900 uppercase">Total Payroll Cost</td>
                <td class="px-6 py-4 text-right text-lg text-indigo-700">${{ totalNetPayroll.toFixed(2) }}</td>
...
const runPayroll = async () => {
  if (!confirm(`Are you sure you want to generate/re-calculate the payroll for ${targetMonth.value}?`)) return
  ...
  try {
    ...
    alert(error.data?.message || 'Failed to calculate payroll')
  }
}
