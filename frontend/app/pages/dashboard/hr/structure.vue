  ...
  <div class="min-h-screen bg-gray-100 dark:bg-gray-700 p-4 sm:p-6 lg:p-8">
    <div class="max-w-7xl mx-auto">
      <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-8">
        <div>
          <h1 class="text-3xl font-bold text-gray-900 dark:text-white tracking-tight">{{ $t('structure.title') }}</h1>
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $t('structure.description') }}</p>
        </div>
      ...
      <div class="bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 rounded-xl overflow-hidden mb-8">
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-900">
              <tr>
                <th scope="col" class="px-6 py-4 text-start text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ $t('structure.department_name') }}</th>
                <th scope="col" class="px-6 py-4 text-start text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ $t('structure.code') }}</th>
                <th scope="col" class="px-6 py-4 text-start text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ $t('structure.manager') }}</th>
                <th scope="col" class="px-6 py-4 text-end text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ $t('structure.actions') }}</th>
              </tr>
            </thead>
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
              <tr v-for="dept in sortedDepartments" :key="dept._id" class="hover:bg-gray-50 dark:bg-gray-900 transition-colors">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl text-start overflow-hidden shadow-2xl w-full max-w-md border border-gray-100 dark:border-gray-700 flex flex-col max-h-[90vh]">
          <form @submit.prevent="saveDepartment" class="flex flex-col flex-1 overflow-hidden">
            <div class="bg-white dark:bg-gray-800 px-6 pt-6 pb-6 overflow-y-auto flex-1">
              <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-6">{{ isEditing ? $t('structure.edit_department') : $t('structure.create_department') }}</h3>
              
              <div class="space-y-4">
                <div>
                  <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{{ $t('structure.department_name_req') }}</label>
                  <input v-model="form.name" type="text" required class="block w-full border border-gray-300 dark:border-gray-600 rounded-lg py-2 px-3 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                </div>
                <div>
                  <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{{ $t('structure.code_req') }}</label>
                  <input v-model="form.code" type="text" required class="block w-full border border-gray-300 dark:border-gray-600 rounded-lg py-2 px-3 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm font-mono uppercase">
                </div>
                <div>
                  <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{{ $t('structure.parent_department') }}</label>
                  <select v-model="form.parentDepartmentId" class="block w-full border border-gray-300 dark:border-gray-600 rounded-lg py-2 px-3 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                ...
                <div>
                  <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{{ $t('structure.manager') }}</label>
                  <select v-model="form.managerId" class="block w-full border border-gray-300 dark:border-gray-600 rounded-lg py-2 px-3 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    <option :value="null">{{ $t('structure.unassigned') }}</option>
                    <option v-for="emp in employees" :key="emp._id" :value="emp._id">{{ emp.name }}</option>
                  </select>
                </div>
                <div>
                  <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{{ $t('structure.dept_description') }}</label>
                  <textarea v-model="form.description" rows="3" class="block w-full border border-gray-300 dark:border-gray-600 rounded-lg py-2 px-3 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"></textarea>
                </div>
              </div>
            </div>
            <div class="bg-gray-50 dark:bg-gray-900 px-6 py-4 flex flex-row-reverse border-t border-gray-100 dark:border-gray-700 flex-shrink-0 gap-3">
              ...
              <button type="button" @click="showModal = false" class="inline-flex justify-center rounded-lg border border-gray-300 dark:border-gray-600 px-4 py-2 shrink-0 whitespace-nowrap bg-white dark:bg-gray-800 text-sm font-bold text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:bg-gray-900">
                {{ $t('structure.cancel') }}
              </button>
            </div>
          </form>
