  ...
  <div class="min-h-screen bg-gray-100 p-4 sm:p-8">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
      ...
      <div class="flex flex-wrap gap-3 mt-4 sm:mt-0 flex-row-reverse">
        ...
        <button @click="openAddStockModal" class="bg-secondary-600 hover:bg-secondary-700 text-white px-4 py-2 rounded-xl font-bold transition-all shadow-sm flex items-center text-sm">
          ...
          <tbody class="bg-white divide-y divide-gray-100">
            <tr v-for="m in movements" :key="m._id" class="hover:bg-gray-50 transition-colors">
              ...
              <td class="px-4 py-3 sm:px-6 sm:py-4 whitespace-nowrap">
                <span v-if="m.type === 'in'" class="px-2.5 py-1 bg-secondary-100 text-secondary-800 text-xs font-bold rounded-full border border-secondary-200">{{ $t('stock_movements.in_restock') }}</span>
              ...
              <td class="px-4 py-3 sm:px-6 sm:py-4 whitespace-nowrap text-sm text-end font-bold" :class="{'text-secondary-600': m.type === 'in', 'text-red-600': m.type !== 'in'}">
                {{ m.type === 'in' ? '+' : '-' }}{{ m.quantity }}
              </td>
              <td class="px-4 py-3 sm:px-6 sm:py-4 text-sm text-gray-500">
                <p>{{ m.reason || $t('stock_movements.n_a') }}</p>
                <p v-if="m.supplierId" class="text-xs text-primary-600 font-bold mt-1">{{ $t('stock_movements.supplier') }} {{ m.supplierId.name }}</p>
        ...
        <div class="p-6 overflow-y-auto flex-1">
          <form @submit.prevent="submitAddStock" class="space-y-4">
            <div>
              ...
              <select v-model="addForm.productId" required class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <input v-model="addForm.quantity" type="number" min="1" required class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500">
              </div>
              <div>
                <label class="block text-sm font-bold text-gray-700 mb-1">{{ $t('stock_movements.unit_cost') }}</label>
                <input v-model="addForm.unitCost" type="number" step="0.01" min="0" required class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500">
              </div>
            </div>

            <div>
              <label class="block text-sm font-bold text-gray-700 mb-1">{{ $t('stock_movements.supplier_label') }}</label>
              <select v-model="addForm.supplierId" class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <input v-model="addForm.referenceNumber" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500">
              </div>
              <div>
                <label class="block text-sm font-bold text-gray-700 mb-1">{{ $t('stock_movements.payment_method') }}</label>
                <select v-model="addForm.paymentMethod" class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500">
            ...
            <div v-if="addForm.quantity && addForm.unitCost" class="p-3 bg-primary-50 text-primary-900 rounded-xl text-sm font-bold border border-primary-100 flex justify-between">
              <span>{{ $t('stock_movements.total_purchase_cost') }}</span>
              <span>${{ (addForm.quantity * addForm.unitCost).toFixed(2) }}</span>
            </div>

            <button type="submit" class="w-full px-4 py-3 bg-secondary-600 text-white rounded-xl font-bold hover:bg-secondary-700 mt-4">{{ $t('stock_movements.confirm_restock') }}</button>
          </form>
        </div>
      </div>
    </div>
      ...
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div class="p-6 overflow-y-auto flex-1">
          ...
          <form @submit.prevent="submitRemoveStock" class="space-y-4">
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-1">{{ $t('stock_movements.select_product') }}</label>
              <select v-model="removeForm.productId" required class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500">
            ...
            <div>
              ...
              <input v-model="removeForm.quantity" type="number" min="1" required class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500">
            </div>

            <div>
              <label class="block text-sm font-bold text-gray-700 mb-1">{{ $t('stock_movements.reason_write_off') }}</label>
              <select v-model="removeForm.reason" required class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-primary-500 focus:border-primary-500 mb-2">
