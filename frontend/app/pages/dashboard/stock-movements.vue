<template>
  <div class="min-h-screen bg-gray-100 p-4 sm:p-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
      <h2 class="text-2xl font-black text-gray-900 tracking-tight">Stock Movements Audit</h2>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
      <div class="px-6 py-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
        <h3 class="font-bold text-gray-900">Historical Log</h3>
      </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Reason / Reference</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="m in movements" :key="m._id" class="hover:bg-gray-50 transition-colors">
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">{{ new Date(m.date).toLocaleString() }}</td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span v-if="m.type === 'in'" class="px-2.5 py-1 bg-emerald-100 text-emerald-800 text-xs font-bold rounded-full border border-emerald-200">IN (Restock)</span>
                <span v-else-if="m.type === 'out'" class="px-2.5 py-1 bg-blue-100 text-blue-800 text-xs font-bold rounded-full border border-blue-200">OUT (Sale)</span>
                <span v-else class="px-2.5 py-1 bg-red-100 text-red-800 text-xs font-bold rounded-full border border-red-200">ADJUST (Loss)</span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ m.productId?.name || 'Unknown' }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-right font-bold" :class="{'text-emerald-600': m.type === 'in', 'text-red-600': m.type !== 'in'}">
                {{ m.type === 'in' ? '+' : '-' }}{{ m.quantity }}
              </td>
              <td class="px-6 py-4 text-sm text-gray-500">
                <p>{{ m.reason || 'N/A' }}</p>
                <p v-if="m.supplierId" class="text-xs text-indigo-600 font-bold mt-1">Supplier: {{ m.supplierId.name }}</p>
                <p v-if="m.referenceNumber" class="text-xs">Ref: {{ m.referenceNumber }}</p>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ m.performedBy?.name || 'System' }}</td>
            </tr>
            <tr v-if="movements.length === 0">
              <td colspan="6" class="px-6 py-8 text-center text-gray-500">No stock movements recorded yet.</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div v-if="showAddModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
          <h3 class="text-lg font-bold text-gray-900">Restock Product (Purchase)</h3>
          ...
          <form @submit.prevent="submitAddStock" class="space-y-4">
            <div>
              ...
              <select v-model="addForm.productId" required class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-indigo-500 focus:border-indigo-500">
                <option v-for="p in products" :key="p._id" :value="p._id">{{ p.name }} (Current: {{ p.stockQuantity }})</option>
              </select>
            </div>
            
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-bold text-gray-700 mb-1">Quantity Added *</label>
              ...
              <div>
                <label class="block text-sm font-bold text-gray-700 mb-1">Unit Cost ($) *</label>
            ...
            <div>
              ...
              <select v-model="addForm.supplierId" class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-indigo-500 focus:border-indigo-500">
                <option :value="null">None (Direct Add)</option>
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-bold text-gray-700 mb-1">Reference (Invoice #)</label>
              ...
              <div>
                ...
                <select v-model="addForm.paymentMethod" class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-indigo-500 focus:border-indigo-500">
                  ...
                  <option value="Credit" :disabled="!addForm.supplierId">Credit (Accounts Payable)</option>
                </select>
              </div>
            </div>
            ...
            <button type="submit" class="w-full px-4 py-3 bg-emerald-600 text-white rounded-xl font-bold hover:bg-emerald-700 mt-4">Confirm Restock</button>
          </form>
        </div>
      </div>
    </div>
    ...
    <div v-if="showRemoveModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
          <h3 class="text-lg font-bold text-red-600">Inventory Adjustment (Loss)</h3>
        ...
        <div class="p-6">
          <div class="mb-4 text-xs font-medium text-gray-500 bg-gray-100 p-3 rounded-lg">
            Use this to write-off damaged, expired, or missing stock. This creates an expense journal entry.
          </div>
          <form @submit.prevent="submitRemoveStock" class="space-y-4">
            <div>
              ...
              <select v-model="removeForm.productId" required class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-indigo-500 focus:border-indigo-500">
                <option v-for="p in products" :key="p._id" :value="p._id">{{ p.name }} (Available: {{ p.stockQuantity }})</option>
              </select>
            </div>
            
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-1">Quantity Lost/Removed *</label>
            ...
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-1">Reason for Write-off *</label>
              <select v-model="removeForm.reason" required class="w-full px-3 py-2 border border-gray-300 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 mb-2">
                ...
                <option value="Inventory Audit Loss">Inventory Audit Loss</option>
                <option value="Theft">Theft</option>
                <option value="Other">Other</option>
              </select>
            </div>

            <button type="submit" class="w-full px-4 py-3 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 mt-4">Confirm Write-off</button>
          </form>
        </div>
      </div>
    </div>
...
const submitAddStock = async () => {
  try {
    ...
    alert('Restock recorded and Journal Entry generated!')
  } catch (error) {
    alert('Failed to add stock: ' + (error.data?.message || 'Unknown error'))
  }
}
...
const submitRemoveStock = async () => {
  try {
    ...
    fetchData()
    alert('Stock write-off completed!')
  } catch (error) {
    alert('Failed to remove stock: ' + (error.data?.message || 'Unknown error'))
  }
}
