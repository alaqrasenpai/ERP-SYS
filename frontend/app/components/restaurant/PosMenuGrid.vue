<template>
  <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-5 pb-10">
    <div v-for="item in menu" :key="item._id" @click="$emit('add-to-cart', item)"
            class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-4 cursor-pointer hover:border-emerald-300 hover:shadow-lg transition-all text-start flex flex-col group relative h-[200px]"
            :class="{ 'opacity-50 pointer-events-none grayscale': item.isAvailable === false }">
      
      <div class="h-20 w-20 mx-auto rounded-full bg-gray-50 dark:bg-gray-900 overflow-hidden border-2 border-white shadow-sm mb-3 shrink-0 relative">
      ...
      <div class="text-center flex-1 flex flex-col justify-end">
        <h3 class="font-black text-gray-900 dark:text-white text-sm leading-tight mb-1 line-clamp-2 group-hover:text-emerald-600 transition-colors">{{ locale === 'ar' ? item.nameAr : item.nameEn }}</h3>
