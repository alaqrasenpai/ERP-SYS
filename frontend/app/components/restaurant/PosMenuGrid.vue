<template>
  <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-5 pb-10">
    <div v-for="item in menu" :key="item._id" @click="$emit('add-to-cart', item)"
            class="bg-white rounded-2xl border border-gray-200 p-4 cursor-pointer hover:border-secondary-300 hover:shadow-lg transition-all text-start flex flex-col group relative h-[200px]"
      ...
      <div class="h-20 w-20 mx-auto rounded-full bg-gray-50 overflow-hidden border-2 border-white shadow-sm mb-3 shrink-0 relative">
        ...
        <div v-else class="w-full h-full flex items-center justify-center text-secondary-500 bg-secondary-50">
      ...
      <div class="text-center flex-1 flex flex-col justify-end">
        <h3 class="font-black text-gray-900 text-sm leading-tight mb-1 line-clamp-2 group-hover:text-secondary-600 transition-colors">{{ locale === 'ar' ? item.nameAr : item.nameEn }}</h3>
        
        <div class="mt-auto pt-2">
          <span class="text-base font-black text-secondary-600 block">{{ item.price.toFixed(2) }} {{ useCookie('erp_currency').value || 'SAR' }}</span>
        </div>
      </div>
    </div>
  </div>
