<template>
  <div class="w-[420px] bg-white border-e border-gray-100 shadow-[4px_0_24px_rgba(0,0,0,0.02)] flex flex-col z-10 shrink-0 dir-rtl">
    ...
    <div class="p-4 border-b border-gray-100 space-y-4">
      <div v-if="orderType === 'Delivery'" class="space-y-3">
        <div>
          <label class="block text-[10px] font-bold text-gray-400 uppercase mb-1">{{ t('restaurant.delivery_provider', 'مزود خدمة التوصيل') }}</label>
          <select :value="deliveryDetails.providerId" @input="$emit('update:provider', $event.target.value)" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-sm font-bold text-gray-700 focus:ring-emerald-500 focus:border-emerald-500">
            <option value="">{{ t('restaurant.select_provider', 'اختر المزود...') }}</option>
        ...
        <div>
          <label class="block text-[10px] font-bold text-gray-400 uppercase mb-1">{{ t('restaurant.external_order_id', 'رقم الطلب الخارجي') }}</label>
          <input :value="deliveryDetails.externalOrderId" @input="$emit('update:externalId', $event.target.value)" :placeholder="t('restaurant.external_id_placeholder', 'مثال: #12345')" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm font-bold text-gray-700 focus:ring-emerald-500 focus:border-emerald-500">
        </div>
      </div>

      <div v-if="orderType === 'Dine-In'" class="space-y-3">
        <div>
          <label class="block text-[10px] font-bold text-gray-400 uppercase mb-1">{{ t('restaurant.table_number', 'رقم الطاولة') }}</label>
    ...
    <div class="flex-1 overflow-y-auto p-4 bg-gray-50/30">
      <div v-if="cart.length === 0" class="h-full flex flex-col items-center justify-center text-gray-300">
        ...
        <p class="font-bold text-sm">{{ t('pos.cart_empty', 'السلة فارغة') }}</p>
      </div>
        ...
        <li v-for="(item, index) in cart" :key="index" class="bg-white p-3 rounded-xl border border-gray-100 shadow-sm flex flex-col">
          <div class="flex justify-between items-start mb-2">
            <div>
              ...
              <div class="text-xs text-gray-400 mt-0.5">{{ item.price.toFixed(2) }} {{ $t('currency', 'ريال') }}</div>
            </div>
          ...
          <input v-model="item.notes" type="text" :placeholder="t('restaurant.add_notes', 'إضافة ملاحظات (بدون بصل، حار...)')" class="w-full text-xs px-3 py-1.5 bg-gray-50/50 border border-gray-100 rounded-lg focus:outline-none focus:border-emerald-400 focus:ring-1 focus:ring-emerald-400 mb-2 text-gray-600">
          ...
          <div class="flex justify-between items-center mt-auto pt-2 border-t border-gray-50">
            ...
            <div class="font-black text-emerald-600 text-sm">
              {{ (item.price * item.quantity).toFixed(2) }} {{ $t('currency', 'ريال') }}
            </div>
          </div>
        </li>
      </ul>
    ...
    <div class="bg-white p-5 border-t border-gray-100 shadow-[0_-10px_30px_rgba(0,0,0,0.02)] shrink-0">
      <div class="space-y-3 mb-6">
        <div class="flex justify-between items-center text-sm">
          <span class="text-gray-500 font-bold">{{ t('pos.subtotal', 'المجموع الفرعي') }}:</span>
          <span class="font-bold text-gray-900">{{ subTotal.toFixed(2) }} {{ $t('currency', 'ريال') }}</span>
        </div>
        ...
        <div class="flex justify-between items-center text-sm">
          ...
          <div class="w-24 relative">
            ...
            <span class="absolute left-2 top-1/2 -translate-y-1/2 text-xs text-gray-400 font-bold">{{ $t('currency', 'ريال') }}</span>
          </div>
        </div>

        <div class="flex justify-between items-center text-sm">
          <span class="text-gray-500 font-bold text-xs">{{ t('pos.tax', 'الضريبة') }} ({{ taxRate }}%):</span>
          <span class="font-bold text-gray-500 text-xs">{{ tax.toFixed(2) }} {{ $t('currency', 'ريال') }}</span>
        </div>
        
        <div class="pt-4 border-t border-gray-100 flex justify-between items-end">
          <span class="text-base font-black text-gray-900">{{ t('pos.total', 'الإجمالي') }}</span>
          <span class="text-2xl font-black text-emerald-500">{{ grandTotal.toFixed(2) }} <span class="text-sm text-emerald-600/60">{{ $t('currency', 'ريال') }}</span></span>
        </div>
      </div>
      ...
      <button v-if="orderType === 'Delivery'" @click="$emit('checkout', 'Aggregator Credit')" :disabled="cart.length === 0" class="w-full py-3.5 bg-[#81C7A1] disabled:bg-gray-200 disabled:text-gray-400 text-white rounded-xl font-black shadow-lg hover:bg-emerald-400 transition flex items-center justify-center">
        {{ t('restaurant.confirm_delivery', 'تثبيت وعرض الفاتورة') }}
      </button>
    </div>
  </div>
</template>
...
defineEmits(['checkout', 'update:qty', 'update:table', 'update:provider', 'update:externalId', 'update:discount'])
</script>
