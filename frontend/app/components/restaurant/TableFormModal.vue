  ...
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
    <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all">
      ...
      <div class="p-6 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-900/50">
        <h3 class="text-xl font-black text-gray-900 dark:text-white">{{ isEditing ? $t('restaurant.edit_table') : $t('restaurant.add_table') }}</h3>
        <button @click="close" class="text-gray-400 hover:text-gray-600 dark:text-gray-400 transition p-2 hover:bg-gray-100 dark:bg-gray-700 rounded-full">
      ...
      <form @submit.prevent="submit" class="p-6 space-y-5">
        <div>
          <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">{{ $t('restaurant.table_number') }}</label>
          <input v-model="form.tableNumber" required class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-gray-900 focus:border-gray-900 transition-all placeholder-gray-400 font-medium" />
        </div>

        <div>
          <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">{{ $t('restaurant.capacity') }}</label>
          <input type="number" v-model="form.capacity" required class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-gray-900 focus:border-gray-900 transition-all placeholder-gray-400 font-medium" />
        </div>

        <div>
          <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">{{ $t('restaurant.section') }}</label>
          <select v-model="form.section" required class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-gray-900 focus:border-gray-900 transition-all font-medium">
        ...
        <div>
          <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">{{ $t('restaurant.status') }}</label>
          <select v-model="form.status" required class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-gray-900 focus:border-gray-900 transition-all font-medium">
        ...
        <div class="pt-4 flex justify-end space-x-3 rtl:space-x-reverse">
          <button type="button" @click="close" class="px-5 py-2.5 rounded-xl font-bold text-gray-600 dark:text-gray-400 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:bg-gray-900 transition">
            {{ $t('restaurant.cancel') }}
          </button>
