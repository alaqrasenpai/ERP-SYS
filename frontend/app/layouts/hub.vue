  ...
  <div class="min-h-[100dvh] bg-gray-50 dark:bg-gray-900 font-sans flex flex-col">
    ...
    <header class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 h-16 flex items-center justify-between px-6 z-30 sticky top-0">
      ...
      <div class="flex items-center gap-4">
        <button @click="setLocale($i18n.locale === 'en' ? 'ar' : 'en')" class="text-sm font-bold text-gray-600 dark:text-gray-400 hover:text-indigo-600 transition-colors px-2 py-1 flex items-center gap-1.5">
          {{ $i18n.locale === 'en' ? 'عربي' : 'English' }}
        </button>
        <button @click="$colorMode.preference = $colorMode.value === 'dark' ? 'light' : 'dark'" class="p-2 text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
